...
TEST(Vector4DHelper, bVec4IsA4DBoolVector)
{
...
TEST(Vector4DHelper, iVec4IsA4DIntVector)
{
...
TEST(Vector4DHelper, uVec4IsA4DUnsignedIntVector)
{
	static_assert(std::is_same_v<math::uVec4::value_type, unsigned int> && "uVec4 must contain unsigned int elements");
	static_assert(std::is_same_v<math::uVec4, math::Vector4D<unsigned int>> && "uVec4 must be an alias of Vector4D<unsigned int>");
}

TEST(Vector4DHelper, vec4IsA4DFloatVector)
{
...
TEST(Vector4DHelper, lVec4IsA4DLongLongVector)
{
	static_assert(std::is_same_v<math::lVec4::value_type, long long> && "lVec4 must contain long long elements");
	static_assert(std::is_same_v<math::lVec4, math::Vector4D<long long>> && "lVec4 must be an alias of Vector4D<long long>");
}

TEST(Vector4DHelper, dVec4IsA4DDoubleVector)
{
...
TEST(Vector4DHelper, ulVec4IsA4DUnsignedLongLongVector)
{
	static_assert(std::is_same_v<math::ulVec4::value_type, unsigned long long> && "ulVec4 must contain unsigned long long elements");
	static_assert(std::is_same_v<math::ulVec4, math::Vector4D<unsigned long long>> && "ulVec4 must be an alias of Vector4D<unsigned long long>");
}